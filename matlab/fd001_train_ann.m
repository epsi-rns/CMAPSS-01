...
%
% Architecture: input -> [64 32] -> 1  (matches Python benchmark)
% Training:     Levenberg-Marquardt (trainlm) — MATLAB default
% Target:       RUL_capped (cap=125 cycles)
%
% MATLAB feedforwardnet convention:
%   Input  X: [features x samples]  (transposed vs Python)
%   Target T: [1 x samples]
%
% Output: fd001_ann_results.mat
...
fprintf('\nTraining Model B — NORM input (15 sensors)...\n');

net_norm = feedforwardnet(hidden_layers, 'trainlm');
net_norm.trainParam.epochs      = max_epochs;
net_norm.trainParam.showWindow  = false;
net_norm.trainParam.show        = 50;
...
function [pred_rul, rmse, mae, nasa] = ...
  ...
  % Evaluate model on test set — get last-cycle prediction per engine

  all_pred = net(X_test')';       % [n_test x 1]
