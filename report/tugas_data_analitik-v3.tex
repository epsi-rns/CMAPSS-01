...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.60\textwidth]{images/01_phm_ecosystem_stack.png}
...
\begin{figure}[H]
  ...
  \caption{Matriks karakteristik empat sub-dataset C-MAPSS ---
    FD001 termudah (1 kondisi, 1 mode kerusakan) hingga FD004 tersulit
    (6 kondisi, 2 mode kerusakan)}
...
\begin{figure}[H]
  ...
  \caption{Skema mesin turbofan C-MAPSS --- sensor mengukur kondisi di sepanjang
    aliran udara dari inlet hingga exhaust; HPC adalah komponen yang terdegradasi}
  \label{fig:engine}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Hubungan antara file train, test, dan RUL ---
    test dipotong sebelum kegagalan; RUL file berisi kunci jawaban}
  \label{fig:train_test_rul}
\end{figure}
...
\begin{figure}[H]
  ...
  \caption{Pipeline normalisasi per kondisi --- raw sensor $\to$ kluster kondisi
    $\to$ statistik healthy $\to$ nilai ternormalisasi}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\textwidth]{images/11_rul_target_shape.png}
  \caption{RUL raw vs RUL capped (cap $= 125$) --- fase sehat diratakan,
    model difokuskan pada fase degradasi}
  \label{fig:rul_cap}
\end{figure}
...
(overestimasi RUL) dihukum lebih berat ($e^{d/13}$) dibandingkan prediksi
\textit{early} ($e^{d/10}$).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.70\textwidth]{images/07_asymmetric_scoring_curve.png}
  \caption{Kurva penalti NASA Score --- prediksi \textit{late} ($d > 0$,
    merah) dihukum lebih berat dari prediksi \textit{early} ($d < 0$, biru)}
  \label{fig:nasa_score}
\end{figure} Pada FD002 dengan 6 kondisi operasi yang
